...
public class JeopardyDAO implements IGameDAO {
	...
	public void persist(BaseEntity entity) {
		play.Logger.info("Entity.class: " + entity.getClass().getName());
		if (entity instanceof Category) {
			Category category = (Category) entity;
			em().persist(this.merge(category));
			for (Question question : category.getQuestions()) {

				em().persist(this.merge(question));
				for (Answer answer : question.getAnswers()) {
					em().persist(this.merge(answer));

				}
			}

		}
		if (entity instanceof JeopardyUser) {
			em().persist(entity);
		}
	}
	...
	public <T extends BaseEntity> T merge(T entity) {
		if (entity.getId() != null) {
			if (findEntity(entity.getId(), entity.getClass()) != null) {
				...
				play.Logger.debug("merge Entity" + entity.getId());
	...
	public <T extends BaseEntity> T findEntity(Long id, Class<T> entityClazz) {
		play.Logger.debug("find Entity: " + id);
